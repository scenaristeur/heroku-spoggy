...
function sheet_to_rdf(sheet, opts) {
	var proprietes=new Array();
	var out = "", txt = "", qreg = /"/g;
	var outOwl="";
	var o = opts == null ? {} : opts;
	
	// filtrer pour verifier les options entrées
	var ontoBase=o;
//	var ontoBasePrefix="<"+ontoBase+"";
	//construction du fichier ttl
	/*var 	outOwl="@prefix : <"+ontoBase+"#> . \n";
			outOwl+="@prefix owl: <http://www.w3.org/2002/07/owl#> . \n";
			outOwl+="@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> . \n";
			outOwl+="@prefix xml: <http://www.w3.org/XML/1998/namespace> . \n";
			outOwl+="@prefix xsd: <http://www.w3.org/2001/XMLSchema#> . \n";
			outOwl+="@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> . \n";
			outOwl+="@base <"+ontoBase+"> . \n";
			outOwl+="\n";
			outOwl+="<"+ontoBase+"> rdf:type owl:Ontology ;\n";
			outOwl+="\n";
			outOwl+="\t\towl:versionIRI <"+ontoBase+"/1.0.0> .\n";
			outOwl+="\n";
			outOwl+="###  Generated by the Smag0 xls2rdf converter (version 0.0.1) http://smag0.blogspot.fr. For more info or question plug my twitter : @DFaveris\n\n";
			*/
	//construction du fichier rdf
	var prefixBase=ontoBase+'#';
	outOwl+='<?xml version="1.0"?>\n\n';
	outOwl+='<!DOCTYPE rdf:RDF [\n';
	outOwl+='    <!ENTITY owl "http://www.w3.org/2002/07/owl#" >\n';
	outOwl+='    <!ENTITY xsd "http://www.w3.org/2001/XMLSchema#" >\n';
	outOwl+='    <!ENTITY rdfs "http://www.w3.org/2000/01/rdf-schema#" >\n';
	outOwl+='    <!ENTITY rdf "http://www.w3.org/1999/02/22-rdf-syntax-ns#" >\n';
	outOwl+=']>\n\n\n';
	outOwl+='<rdf:RDF xmlns="'+ontoBase+'#"\n';
	outOwl+='xml:base="'+ontoBase+'"\n';
	outOwl+='     xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n';
	outOwl+='     xmlns:owl="http://www.w3.org/2002/07/owl#"\n';
	outOwl+='     xmlns:xsd="http://www.w3.org/2001/XMLSchema#"\n';
	outOwl+='     xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#">\n';
	outOwl+='    <owl:Ontology rdf:about="'+ontoBase+'">\n';
	outOwl+='        <owl:versionIRI rdf:resource="'+ontoBase+'/1.0.0"/>\n';
	outOwl+='    </owl:Ontology>\n\n\n';
	outOwl+='';
	outOwl+='';
	...
	//	console.log(rr); 
		for(C = r.s.c; C <= r.e.c; ++C) {
			var outOwlTempClass="";
			var outOwlTempUri="";	
			var outOwlTempObjectProperty="";	
			var outOwlObject="";
			var outOwlLinkSujet2Object="";
			var outOwlTempDataProperty="";
			var outOwlLinkSujet2Data="";
			val = sheet[cols[C] + rr];
			//		console.log(val); // couleur recuperable avec propriete icvBack et icvFore en format xls97
			txt = val !== undefined ? ''+format_cell(val) : "";
			...
			if (rr==1){
				...
				for(i = 0, cc = 0; i !== txt.length; ++i) if((cc = txt.charCodeAt(i)) === fs || cc === rs || cc === 34) {
					...
					var ligne="";
					if (txt !=""){
					var propriete = proprietes[C];

					/* ATTENTION : A GERER SI LE SUJET N'EST PAS DANS LA PREMIERE COLONNE --> retarder traitement des autres colonnes tant qu'on n'a pas le sujet */
					
					
					switch (true) {
						case propriete.startsWith( '!' ):
							//alert(1); // do something
							//console.log("ne pas afficher "+propriete);
						break;
						case propriete.startsWith( 'O_' ):
							//alert(1); // do something
							propriete=propriete.substring(2);
							propriete=format2rdf(propriete);
							var classe=propriete.substr(0,1).toUpperCase()+	propriete.substr(1,propriete.length).toLowerCase()	;
							propriete.toLowerCase();
							//creation de la propriete objet
							outOwlTempObjectProperty+='<!-- '+prefixBase+propriete+' -->\n';
							outOwlTempObjectProperty+='<owl:ObjectProperty rdf:about="'+prefixBase+propriete+'"/>\n\n';					
							//creation de la classe
							outOwlTempObjectProperty+='<!-- '+prefixBase+classe+' -->\n';
							outOwlTempObjectProperty+='<owl:Class rdf:about="'+prefixBase+classe+'"/>\n\n';	
							txt=format2rdf(txt);
							outOwlObject+='<!-- '+prefixBase+txt+' -->\n';
							outOwlObject+='<owl:NamedIndividual rdf:about="'+prefixBase+txt+'">\n';
							outOwlObject+='\t<rdf:type rdf:resource="'+prefixBase+classe+'"/>\n';
							outOwlObject+='</owl:NamedIndividual>\n\n';
							
							//creation du lien
							outOwlLinkSujet2Object+='<!-- '+prefixBase+sujet+' -->\n';
							outOwlLinkSujet2Object+='<owl:NamedIndividual rdf:about="'+prefixBase+sujet+'" > \n';
							outOwlLinkSujet2Object+='<'+propriete+' rdf:resource="'+prefixBase+txt+'"/> \n';
							outOwlLinkSujet2Object+='</owl:NamedIndividual> \n\n';
							
							ligne="\tObjectProperty:\t"+propriete+":\t"+txt;
							break;
							
						case propriete.startsWith( 'D_' ):
							propriete=propriete.substring(2);
							propriete=format2rdf(propriete);
							propriete.toLowerCase();
							//creation de la propriete data
							outOwlTempDataProperty='<!-- '+prefixBase+propriete+' -->\n';
							outOwlTempDataProperty='<owl:DatatypeProperty rdf:about="'+prefixBase+propriete+'"/>\n\n';			
							//creation du lien
					
								txt=txt.replace(/[&]/g,"&amp;");
							outOwlLinkSujet2Data+='<!-- '+prefixBase+sujet+' -->\n';
							outOwlLinkSujet2Data+='<owl:NamedIndividual rdf:about="'+prefixBase+sujet+'" > \n';
							outOwlLinkSujet2Data+='<'+propriete+'>'+txt+'</'+propriete+'> \n';
							outOwlLinkSujet2Data+='</owl:NamedIndividual> \n\n';

						
						
						
							ligne="\tDataProperty:\t"+propriete+":\t"+txt;
							break;
						default : 
							ligne= "\nSUJET : "+ txt +" \trdf:type \t "+ propriete;
							//	:Robot rdf:type owl:Class .
							txt=format2rdf(txt);
							sujet=txt;
							outOwlTempClass+='<!-- '+prefixBase+propriete+' -->\n\n';
							outOwlTempClass+='<owl:Class rdf:about="'+prefixBase+propriete+'" /> \n\n';
							// txt rdf:type propriete
							outOwlTempUri+='<!-- '+prefixBase+txt+' -->\n\n';
							outOwlTempUri+='<owl:NamedIndividual rdf:about="'+prefixBase+txt+'" > \n';
							outOwlTempUri+='<rdf:type rdf:resource="'+prefixBase+propriete+'"/> \n';
							outOwlTempUri+='</owl:NamedIndividual> \n\n';
							
							
							
						/* test pour ttl
								outOwlTemp+="\n### "+ontoBase+"#"+propriete+"\n\n";
								outOwlTemp+=":"+propriete+"\trdf:type owl:Class .\n\n";
						*/
					...
					row += (C === r.s.c ? "" :  "\n"+ligne);
					//row += (C === r.s.c ? "" : RS) + "\t"+ligne;
				//row += (C === r.s.c ? "" : FS) + txt;
				}
	//	console.log( "recherche --"+outOwlTemp+"--\n");
	//	console.log( "dans --"+outOwl+"--\n");
		if (outOwl.indexOf(outOwlTempClass) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlTempClass;
		}
		if (outOwl.indexOf(outOwlTempUri) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlTempUri;
		}
		if (outOwl.indexOf(outOwlTempObjectProperty) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlTempObjectProperty;
		}
		if (outOwl.indexOf(outOwlObject) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlObject;
		}
		if (outOwl.indexOf(outOwlLinkSujet2Object) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlLinkSujet2Object;
		}		
		if (outOwl.indexOf(outOwlTempDataProperty) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlTempDataProperty;
		}
		if (outOwl.indexOf(outOwlLinkSujet2Data) == -1) {
			//ajoute si non inclu
				outOwl+=outOwlLinkSujet2Data;
		}			
	...
	outOwl+='</rdf:RDF> \n';
	outOwl+='\n<!-- Generated by the Smag0 xls2rdf converter (version 0.0.1) http://smag0.blogspot.fr. For more info or question plug my twitter : @DFaveris-->\n';
			console.log(outOwl);
		return outOwl;
/*	out = proprietes+"\n"+out;
	return out;*/
}
var make_rdf = sheet_to_rdf;
...
function format2rdf(txt){
//	txt=txt.replace(" ","_");
//	txt=txt.replace(".","_");
	txt=txt.replace(/[éèê]/g,"e");
		txt=txt.replace(/[âä]/g,"a");
...
var utils = {
	...
	make_csv: sheet_to_csv,
		make_rdf: sheet_to_rdf,
	make_json: sheet_to_json,
	make_formulae: sheet_to_formulae,
	sheet_to_csv: sheet_to_csv,
		sheet_to_rdf: sheet_to_rdf,
