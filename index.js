...
io.on('connection', function (socket) {
  ...
  socket.on('sendchat', function (data) {
    ...
    io.sockets.in(socket.graph).emit('updatechat', socket.username, data);
  });

  socket.on('switchGraph', function(newgraph){
    // leave the current room (stored in session)
    console.log(newgraph);
    graphs.indexOf(newgraph) === -1 ? graphs.push(newgraph) : console.log("This item already exists");
    socket.leave(socket.graph);
    // join new graph, received as function parameter
    socket.join(newgraph);
    socket.emit('updatechat', 'SERVER', 'you have connected to '+ newgraph);
    // sent message to OLD graph
    socket.broadcast.to(socket.graph).emit('updatechat', 'SERVER', socket.username+' has left this graph');
    // update socket session graph title
    socket.graph = newgraph;
    socket.broadcast.to(newgraph).emit('updatechat', 'SERVER', socket.username+' has joined this graph');
    console.log(graphs);
    socket.emit('updategraphs', graphs, newgraph);
    socket.broadcast.emit('updategraphs', graphs );
  });
...
/*  if(useLevelgraph){
  ...
  console.log("ici on devrait INITGRAPH")
}
...
//Socket.IO
//var graphs = ["index", "tension","blip", "blop"];
/*
...
var graph = graphs[0];
updategraph(socket,graph);


socket.on('create', function(graph) {
graphs.push(graph);
console.log(socket.graph);
io.emit("graphs", graphs);
updategraph(this, graph);
});
socket.on('join', function(graph) {
updategraph(this, graph);
});
...
/*
updategraph = function(socket,graph){
if(socket.graph){
socket.leave(socket.graph);
}
socket.graph = graph;
socket.join(graph);
io.in(graph).emit('joined', {username: socket.id, message: graph });
}
*/
